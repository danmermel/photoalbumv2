...
// Single Album API
resource "aws_lambda_function" "singleAlbumAPI" {
  filename      = data.archive_file.lambda.output_path
...
resource "aws_cloudwatch_log_group" "lambdaLGsingleAlbumAPI" {
  name              = "/aws/lambda/${aws_lambda_function.singleAlbumAPI.function_name}"
  retention_in_days = 7
}

resource "aws_lambda_function_url" "singleAlbumAPIFunctionUrl" {
  function_name      = aws_lambda_function.singleAlbumAPI.function_name
  authorization_type = "NONE"
  cors {
    allow_credentials = true
    allow_origins     = ["*"]
...
output "singleAlbumAPIFunctionUrl" {
  value = aws_lambda_function_url.singleAlbumAPIFunctionUrl.function_url
}

// Single Image API
resource "aws_lambda_function" "singleImageAPI" {
  filename      = data.archive_file.lambda.output_path
...
resource "aws_cloudwatch_log_group" "lambdaLGsingleImageAPI" {
  name              = "/aws/lambda/${aws_lambda_function.singleImageAPI.function_name}"
  retention_in_days = 7
}

resource "aws_lambda_function_url" "singleImageAPIFunctionUrl" {
  function_name      = aws_lambda_function.singleImageAPI.function_name
  authorization_type = "NONE"
  cors {
    allow_credentials = true
    allow_origins     = ["*"]
...
output "singleImageAPIFunctionUrl" {
  value = aws_lambda_function_url.singleImageAPIFunctionUrl.function_url
}
